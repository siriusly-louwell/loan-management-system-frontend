    ...
    return (
        <div className="sm:flex space-x-2 my-5">
            ...
            <div className='grid grid-cols-10 lg:grid-cols-9 gap-2 mt-4 sm:mt-0 sm:gap-y-3 sm:gap-x-10'>
                {spectrum.map(color => (
                    <>
                        <label htmlFor={color}><ColorLabel style={color} size={size} selected={colors.includes(`${color}`) ? "border-blue-500 border-2" : ""} /></label>
                        <input type="checkbox" value={color} id={color} className="hidden" check={colors.includes(`${color}`)} onChange={(e) => changeColor(e.target.value)} />
                    </>
                ))}
            </div>
        </div>
